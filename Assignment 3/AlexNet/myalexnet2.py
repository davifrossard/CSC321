...
def alex_net_bp(input_images):

    img = convert_faces(deepcopy(input_images))

    with tf.Graph().as_default() as g:
        with g.gradient_override_map({"Relu": "CustomRelu"}):

            input = tf.placeholder(tf.float32, [1, 227, 227, 3])
            #conv1
        ...
        rinds, rgrads, probs = [], [], []
        for image in img:
            out_prob = sess.run(prob, feed_dict={input: image})

            inds = argsort(out_prob)[0,::-1]
            grads, indsi, probsi = [], [], []
            for i in range(4):
                smax = tf.split(1, 1000, prob)[inds[i]]
                grad = tf.gradients(smax, input)
                sess.run(tf.initialize_all_variables())

                gradient = sess.run(grad[0], feed_dict={input: image})[0]
                gradient /= np.max(gradient)

                grads.append(gradient)
                indsi.append(inds[i])
                probsi.append(out_prob[0,inds[i]])
            rinds.append(indsi)
            rgrads.append(grads)
            probs.append(probsi)

    return rgrads, rinds, probs
...
def get_gradient(images):
    ...
    '''
    grads, inds, probs = alex_net_bp(images)

    for grad, ind, prob, img in zip(grads, inds, probs, images):
        plt.figure()
        plt.suptitle("Guided Backpropagation in AlexNet", size=20)
        plt.subplot(2,2,1)
        plt.imshow(img)
        plt.title("Input")
        plt.axis('off')
        ...
        plt.title("Dream Input")
        plt.axis('off')
        for i, j in enumerate([3,4,7,8]):
            plt.subplot(2,4,j)
            plt.imshow(np.maximum(grad[i],0))
            plt.title("%s (%.2f%%)" %(class_names[ind[i]], prob[i]*100))
            plt.axis('off')
