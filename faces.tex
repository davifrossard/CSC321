...
Its noticeable how the accuracy between the training set and test and validation sets differ far more than in \autoref{t4:k_sweep}. This happens because before we were classifying actors that were in all sets, this way each sample outside the training set had a high likelihood of having a sample in the training set that shared many features, since it is the same person. When the actors on the training set are different than the ones on the validation and test sets we start to rely on inherent features of man and woman to make the classification, which poses as a harder task since the barrier becomes more blurry, evidenced by how the performance changed from \autoref{t5:k_test} to \autoref{t6:k_test}.
